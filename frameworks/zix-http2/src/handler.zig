//! HttpArena: zix
//!
//! Implements a high-performance HTTP/2 server featuring an append-only,
//! lock-free in-memory cache for serving static files
//! with precompressed content negotiation (Brotli/Gzip),
//! alongside optimized handlers for JSON dataset rendering
//! and query parameter aggregation.
...
/// the bytes read into memory once plus its content type.
/// ok is false for a missing file (caches the 404 so a bad path is not re-probed).
/// content_encoding is "" for identity,
...
/// A ".br" / ".gz" suffix reports that encoding,
/// with the content type taken from the stripped name ("vendor.js.br" -> javascript).
...
/// Read a static file fully into a process-lifetime buffer.
...
/// Probe + cache a static path on first request, then return the slot.
/// Caches a not-found slot so a bad path is probed only once.
...
/// Returns the slot only when the file exists on disk (ok),
...
/// Send a 200 body through the flow-controlled streaming writer,
/// which frames it into DATA chunks and
...
fn sendH2File(fd: std.posix.fd_t, sid: u31, content_type: []const u8, content_encoding: []const u8, bytes: []const u8) void {
    // The cached bytes are process-lifetime,
    // the mux may reference and pace them by WINDOW_UPDATE.
    zix.Http2.sendResponseStreamFD(fd, sid, 200, content_type, content_encoding, bytes);
}

// --------------------------------------------------------- //

// GET/POST /baseline2?a=..&b=.. : sum query values plus the POST body as an integer,
// returns text/plain.
...
// GET /json/{count}?m=M :
// render count dataset items, total = price*quantity*M. Body (near 12 KiB) fits
...
// GET /static/{file} :
// serve from /data/static, content type by extension, body cached on first read.
// Negotiates .br then .gz when accepted, else identity.
// Never compresses, only serves a precompressed
