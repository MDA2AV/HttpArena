//! HttpArena: zix-http2
//!
//! zix HTTP/2 entry point on the zix.Http2 engine (no std.http). Two listeners run in parallel:
//! - h2c cleartext on H2C_PORT under .URING (shared-nothing per-core io_uring, one SO_REUSEPORT
//!   listener plus ring per CPU). Serves baseline-h2c and json-h2c.
//! - h2 over TLS 1.3 on H2_TLS_PORT (ALPN h2, self-signed Ed25519 cert at /etc/zix-tls): the per-core
//!   tls_mux (one worker per core, no thread-per-connection). Serves baseline-h2 and static-h2.
//!
//! Endpoints:
//! - GET /baseline2?a=..&b=.. : sum the query values plus the POST body as an integer, text/plain.
//! - GET /json/{count}?m=M    : render count dataset items, total = price*quantity*M, json.
//! - GET /static/{file}       : serve /data/static by extension, body as chunked DATA frames (<= 16 KiB).
//!
//! One route table serves both listeners: extra routes on each port are simply never hit by the
...
fn badRequest(fd: std.posix.fd_t, sid: u31) void {
    zix.Http2.sendResponse(fd, sid, 400, "text/plain", "bad request") catch {};
}
...
fn baselineHandler(method: []const u8, headers: []const zix.Http2.Header, body: []const u8, fd: std.posix.fd_t, sid: u31) void {
...
// the 16 KiB frame cap, so it ships as a single DATA frame via sendResponse.
fn jsonHandler(_: []const u8, headers: []const zix.Http2.Header, _: []const u8, fd: std.posix.fd_t, sid: u31) void {
    const path = pathFromHeaders(headers);
    ...
    zix.Http2.sendResponse(fd, sid, 200, "application/json", buf[0..pos]) catch {};
}
...
/// Static cache capacity: 20 fixtures times their (.br, .gz, identity) candidates plus 404 headroom,
...
// Append-only cache: readers scan 0..count lock-free (count published release-ordered after the slot
// is fully written), the spinlock only serializes inserts (rare, one per distinct path).
...
fn resolveStatic(name: []const u8) ?*const StaticEntry {
    const count = @atomicLoad(usize, &g_static_count, .acquire);
...
fn prewarmStatic() void {
    ...
    // Iterate with raw getdents64 (this std.fs has no portable Dir.iterate). linux_dirent64 layout:
...
const ROUTES = &[_]zix.Http2.Route{
    .{ .path = "/baseline2", .handler = baselineHandler },
...
pub fn main(process: std.process.Init) !void {
    // Elevate scheduling priority (setpriority -19). Fails silently without CAP_SYS_NICE, so no special
    // capability is required for correctness.
    ...
    // Warm the static cache before any worker serves, so the request path is lock-free (no spinlock held
    // across a file read under the oversubscribed worker pool).
    prewarmStatic();
    ...
    // h2 over TLS 1.3 on H2_TLS_PORT (ALPN h2) with the baked Ed25519 cert. The cleartext server runs
    ...
    // h2c cleartext on H2C_PORT under the native io_uring dispatch model.
