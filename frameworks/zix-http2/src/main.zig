...
//!   (ALPN h2, self-signed Ed25519 cert at /etc/zix-tls): the per-core
//!   tls_mux (one worker per core, no thread-per-connection).
//!   Serves baseline-h2 and static-h2.
//!
//! Endpoints:
...
// TLS h2 listener:
// the per-core tls_mux terminates TLS 1.3 (ALPN h2), one worker per core.
// A missing or unreadable cert degrades gracefully:
// this thread returns and the cleartext h2c server keeps running.
