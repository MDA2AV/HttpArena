...
pub const Parser = struct {
    ...
    close: bool = false,
    /// Set from Accept-Encoding header. Lets static / json-comp handlers
    /// pick a pre-compressed variant without re-parsing the header.
    accepts_br: bool = false,
    accepts_gzip: bool = false,

    /// Body accumulator (separate from header buffer to keep header offsets
    /// valid after pipelining shifts). Sized by the caller — for the
    /// HttpArena workload 512 B is plenty; an upload-style server would
    /// allocate megabytes here.
    body: []u8,
    body_len: u32 = 0,
    ...
    /// Suggested header buffer size. The framework passes whatever it
    /// likes; 2 KiB fits the bench's pipelined batch (~1.3 KiB) with
    /// headroom for split-fragment tests.
    pub const DEFAULT_HEADER_BUF: u32 = 2048;
    /// Suggested body buffer size. 512 B handles the HttpArena baseline
    /// POSTs (≤ 4 bytes) and small JSON bodies. Upload-style servers
    /// should allocate larger.
    pub const DEFAULT_BODY_BUF: u32 = 512;
    ...
    pub fn feed(self: *Parser, n: u32) FeedResult {
        self.len += n;
        if (self.len > self.buf.len) return .protocol_error;
        ...
        if (self.is_chunked) {
            ...
            if (self.content_length > self.body.len) return .protocol_error;
        ...
        while (self.chunk_pos < self.len and self.chunk_state != .done) {
            ...
            switch (self.chunk_state) {
                ...
                .data => {
                    ...
                    if (self.body_len + take > self.body.len) return false;
    ...
    fn parseRequestLineAndHeaders(self: *Parser) bool {
        ...
        while (i + 1 < end) {
            ...
            } else if (eqlIgnoreAsciiCase(name, "accept-encoding")) {
                if (containsTokenIgnoreCase(value, "br")) self.accepts_br = true;
                if (containsTokenIgnoreCase(value, "gzip")) self.accepts_gzip = true;
            }
...
test "GET baseline11 simple" {
    ...
    var p: Parser = .{ .buf = &hdr_buf, .body = &body_buf };
