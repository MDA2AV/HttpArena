...
fun main() {
    println("Ktor+Kormium (Kotlin/Native, CIO) HttpArena server starting on :8080 (HTTP/1.1)")
...
private fun Application.configureRouting(appData: ArenaApplicationDeps) {
    val pipelineResponse = ByteArrayContent("ok".encodeToByteArray(), ContentType.Text.Plain)
    ...
    routing {
        ...
        route("/async-db") {
            ...
            get {
                ...
                try {
                    ...
                    log.error("Failed to load items from DB: ${e.message}")
                    call.respondText("{\"items\":[],\"count\":0}", ContentType.Application.Json)
                }
            }
        }
        ...
        get("/fortunes") {
            ...
            try {
                ...
                log.error("Failed to load fortunes from DB: ${e.message}")
                call.respond(HttpStatusCode.InternalServerError, "fortunes failed")
                return@get
            }
...
fun Route.crudEndpoints(appData: ArenaApplicationDeps, log: Logger = KtorSimpleLogger("crudRoutes")): Route =
    route("/crud/items") {
        install(ContentNegotiation) {
            json(appData.json)
        }
        get {
            ...
            try {
                ...
                log.error("CRUD list failed: ${e.message}")
                call.respond(HttpStatusCode.InternalServerError, "list failed")
            }
        }
        ...
        get("{id}") {
            ...
            try {
                ...
                val body = appData.json.encodeToString(DbItem.serializer(), row).encodeToByteArray()
                ...
                log.error("CRUD read failed: ${e.message}")
                call.respond(HttpStatusCode.InternalServerError, "read failed")
            }
        }
        ...
        post {
            ...
            try {
                ...
                log.error("CRUD create failed: ${e.message}")
                call.respond(HttpStatusCode.InternalServerError, "create failed")
            }
        }
        ...
        put("{id}") {
            ...
            try {
                ...
                log.error("CRUD update failed: ${e.message}")
                call.respond(HttpStatusCode.InternalServerError, "update failed")
            }
        }
    }
