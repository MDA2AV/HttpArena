...
fn main() -> wtx::Result<()> {
  ...
  let tls_config = TlsConfig::from_keys_pem(&*cert_file, &mut rng, &*key_file)?;
...
async fn endpoint_grpc_stream(
  mut ms: ManualStream<GrpcManager<QuickProtobuf>, ServerStream<OwnedWriteHalf, SkCtx>>,
) -> wtx::Result<()> {
  ...
  common.send_data_concurrent::<_, 50>((0..sr.count).map(|_| &bytes[..])).await?;
  Ok(())
}
