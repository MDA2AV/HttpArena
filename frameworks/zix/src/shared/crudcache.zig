//! In-process cache for crud reads. Direct-mapped item slots (x-cache
//! MISS/HIT, invalidated on write) plus scanned list-page slots (TTL-bounded
//! staleness only, single-flight refresh past the TTL). Per-slot and
//! per-list spinlocks, slots live in BSS.
