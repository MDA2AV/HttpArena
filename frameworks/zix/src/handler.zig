...
pub fn rawIntercept(rem: []const u8, header_end: usize, fd: std.posix.fd_t) ?usize {
    ...
    // (each a bodyless GET, length = its header end + 4).
    // Stop at the first non-/pipeline or incomplete request
    // and let the engine resume from there.
    while (true) {
