//! tokio — a minimal HTTP/1.1 server on raw tokio for the H1-isolated profiles
//! (baseline, pipelined, limited-conn). No HTTP framework: the request parser
//! (request line, headers, Content-Length + chunked bodies, keep-alive,
//! pipelining, fragmented reads) is hand-rolled on a tokio TcpStream.
//!
//! Serving model: one current-thread runtime per core, each with its own
//! SO_REUSEPORT listener. Listens on 0.0.0.0:8080 (PORT overrides for testing).
//!
//! Endpoints:
