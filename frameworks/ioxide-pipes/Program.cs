...
internal static class Program
{
    ...
    private static void Main()
    {
        ...
        //
        // KernelTx is the one TLS knob moved off its default. The plaintext goes into the write
        // slab and the kernel makes the records, which is one pass over the bytes instead of
        // OpenSSL's encrypt-then-copy. Nothing above changes: TlsConnectionDualPipe pairs its
        // halves from the SESSION, so it picks the plain writer here and the encrypting one
        // otherwise, and the serve loop cannot tell.
        //
