...
internal sealed class Config : IDisposable
{
    ...
    public static Config FromEnvironment()
    {
        ...
        var server = new ServerConfig
        {
            ...
            // ENTRY DEFAULT: 256 KB x 16 rather than 16 KB x 256. Identical ring bytes per
            // reactor (4 MB), reshaped - a 100 KB echo body lands in one recv instead of seven,
            // worth ~15% on echo-100k with the server saturated. Measured against 128 KB x 32
            // over three pairs (+2.2%, consistent) and neutral where it does not matter:
            // baseline at 4096 connections and json-tls both inside noise, and 16 buffers per
            // reactor showed no starvation at 4096 connections.
            RecvBufferSize = Env.Int("IOXIDE_RECV_BUF_KB", 256) * 1024,
            RecvSlots      = Env.Int("IOXIDE_RECV_SLOTS", 16),
