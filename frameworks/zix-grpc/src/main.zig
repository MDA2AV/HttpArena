...
//! - h2c cleartext on PORT (8080) under the .URING dispatch model.
//!   Serves unary-grpc and stream-grpc.
//! - gRPC over TLS 1.3 on TLS_PORT (8443),
//!   ALPN h2, with a self-signed Ed25519 cert baked at
//!   /etc/zix-tls. Serves unary-grpc-tls and stream-grpc-tls.
//!   The TLS path is the per-core tls_mux 
//!   (one worker per core, no thread-per-connection),
//!   shared by the .EPOLL and .URING models.
//!
//! Design choices:
...
// gRPC over TLS 1.3 listener (per-core tls_mux, ALPN h2).
// A missing cert degrades gracefully:
// this thread returns and the cleartext server keeps running.
