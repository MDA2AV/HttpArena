...
/// <summary>
/// shrike-tokio — an epoll engine with an IVTS-backed, RCA=true async handler loop,
/// using Tokio's readiness model: the worker thread is a pure READINESS notifier
/// (epoll → SignalReadable; it never touches the socket), and the per-connection
/// handler resumes on the THREAD POOL and does its OWN recv() (Connection.DoRecv).
/// Because only the handler ever touches the recv buffer, there is no driver/handler
/// race — unlike a "worker recv's then hands off" design. FlushAsync sends directly,
/// so the off-worker handler needs no handoff. A fully async-work-ready architecture.
///
...
internal static class Program
{
    ...
    /// <summary>
    /// Per-connection handler. RCA=true → each <c>await</c> resumes on the thread
    /// pool. ReadAsync waits for READINESS; the handler then recv's itself (DoRecv),
    /// so the recv buffer is touched only by this thread.
    /// </summary>
    private static async Task HandleAsync(Connection conn)
    {
        while (true)
        {
            ...
            if (!conn.DoRecv())                // recv on THIS (handler) thread — no shared buffer
                return;                        // peer closed / hard error

            bool wrote = DrainRequests(conn, out bool close);
