...
| Concurrency | `async` * | `GET /delay/{ms}` with the wait named in the route, 16K held connections. Isolates what a framework does while a request is pending, with no database or I/O in the way. Reference-only for now: measured and shown, but not yet part of the composite score |
