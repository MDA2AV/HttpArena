...
| Efficiency | `latency-1m`, `latency-10k` * | One million req/s offered at a fixed rate with zrk; the metric is CPU spent per request, read exactly from the container's cgroup rather than sampled. Everyone who finishes serves the same million, so what separates them is the bill. Scored on its own 0-100 formula rather than on rps, since the rate is pinned. `latency-10k` is the identical setup - same cores, connections and endpoint - at a near-idle 10K req/s, so what it reports is the standing cost of being a running server rather than of serving load; reference-only for now |
| Workload | `json-comp`, `json-tls`, `echo-100k` *, `static-tls` | JSON serialization, gzip/brotli compression, HTTP/1.1 over TLS, a 100 KB body echoed over TLS so ingest and egress load together, 20-file static asset serving over TLS |
