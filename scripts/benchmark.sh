...
for t in baseline pipelined limited-conn json-comp json-tls upload \
         static-tls async-db async latency-1m latency-10k latency-500k-4cpu \
...
if { framework_subscribes_to "latency-1m" || framework_subscribes_to "latency-10k" || framework_subscribes_to "latency-500k-4cpu" || framework_subscribes_to "8gbit"; } && [ -z "${ZRK_CMD:-}" ]    && ! command -v "$ZRK" >/dev/null 2>&1; then
...
run_one() {
    ...
    local _mdir=""
    case "$endpoint" in latency-1m|latency-10k|latency-500k-4cpu|8gbit) _mdir=$(mktemp -d) ;; esac
    ...
    if [ "$endpoint" = "latency-1m" ] || [ "$endpoint" = "latency-10k" ] || [ "$endpoint" = "latency-500k-4cpu" ] || [ "$endpoint" = "8gbit" ]; then
...
save_result() {
    ...
    local eff_extra=""
    if [ "$profile" = "latency-1m" ] || [ "$profile" = "latency-10k" ] || [ "$profile" = "latency-500k-4cpu" ] || [ "$profile" = "8gbit" ]; then
