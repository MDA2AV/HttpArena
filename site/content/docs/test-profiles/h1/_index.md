...
{{< cards >}}
  ...
  {{< card link="latency-500k-4cpu" title="Latency-500K/4 *" subtitle="500K req/s pinned with the server on two cores plus their SMT siblings - near saturation, so the queue a server builds shows in its mean latency. Reference-only while the first entries are measured." icon="chip" >}}
