...
On the composite board this score is normalized against the leading score in the field and contributes on the 0-1,000 basis every profile column uses, so the leader here is worth 1,000 just as the leader of an rps column is. Computed by [`scripts/latency_score.py`](https://github.com/MDA2AV/HttpArena/blob/main/scripts/latency_score.py), mirrored in the board's JavaScript; run `python3 scripts/latency_score.py --table --profile latency-1m` for the published results.
