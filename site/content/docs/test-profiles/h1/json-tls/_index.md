...
A dataset of 50 items serialized to JSON with a per-request multiplier, transported over **HTTP/1.1 on top of TLS** through a dedicated port (8081). This is the reference definition of the `/json` endpoint. Measures how much of a framework's plaintext throughput survives TLS record framing, symmetric cipher work, and record-boundary scheduling.
