...
At 20 ms and 64,000 connections that is **3.20M rps**. The best entry measured so far is 2.39M, so there is headroom, but not a great deal of it: an entry closing on 3.20M is running out of profile rather than running out of capacity, and the number should be read with that in mind.
...
A 64-thread server at 20 ms tops out near 3,200 rps no matter how many connections are offered, against millions for an entry that suspends the request instead.
...
- **Timer granularity is priced in.** The delay is 20 ms, so a runtime that habitually overshoots by 1-2 ms is handing back a twentieth to a tenth of the wait before it has served a single request. That cost is real and the profile charges for it. Validation never asserts an upper bound on a single response, though - only the benchmark prices it
